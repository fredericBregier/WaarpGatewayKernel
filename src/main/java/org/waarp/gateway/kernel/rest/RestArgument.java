 ...
 * over:</br> - ARG_HASBODY, ARG_METHOD, ARG_PATH,
 * ARG_BASEPATH, ARGS_SUBPATH, ARG_X_AUTH_KEY, ARG_X_AUTH_USER,
 * ARG_X_AUTH_TIMESTAMP, ARG_X_AUTH_ROLE: root elements
 * (query only)</br> - ARG_METHOD, ARG_PATH, ARG_BASEPATH, ARGS_SUBPATH,
 * ARG_X_AUTH_USER, JSON_STATUSMESSAGE,
 * JSON_STATUSCODE, JSON_COMMAND: root elements (answer only)</br> - ARGS_URI:
 * uri elements (query only)</br> -
 * ARGS_HEADER: header elements (query only)</br> - ARG_COOKIE: cookie
 * elements</br> - ARGS_BODY: body elements (query
...
public class RestArgument {
   ...
   *
   * @throws HttpInvalidAuthenticationException if the computation of the
  ...
  public void setFromArgument(RestArgument source) {
    ...
    if (source.arguments.has(REST_ROOT_FIELD.ARG_PATH.field)) {
      arguments
          .put(REST_ROOT_FIELD.ARG_PATH.field,
               source.arguments.get(REST_ROOT_FIELD.ARG_PATH.field).asText());
    }
   ...
   * in the URI or Header<br> 0) Check that
   * timestamp is correct (|curtime - timestamp| < maxinterval) from
   * ARG_X_AUTH_TIMESTAMP, if maxInterval is 0, not
   * mandatory<br> 1) Get all URI args (except ARG_X_AUTH_KEY itself, but
   * including timestamp), lowered case, in
   * alphabetic order<br> 2) Add an extra Key if not null (from
   * ARG_X_AUTH_INTERNALKEY)<br> 3) Compute an hash (SHA-1
  ...
  public static enum REST_ROOT_FIELD {
     ...
     * arguments.path(ARGS_SUBPATH).elements()
