...
public abstract class DataModelRestMethodHandler<E extends AbstractDbData>
  ...
  /**
   * For Read or Update, should include a select() from the database. Shall not
   * be used for Create. JSON_ID should be
  ...
  protected abstract DbPreparedStatement getPreparedStatement(
      HttpRestHandler handler, RestArgument arguments,
      RestArgument result, Object body)
      throws HttpIncorrectRequestException,
             HttpInvalidAuthenticationException;
   ...
   * @param statement
   *
   * @return the Object E according to statement (using next) or null if no more
   *     item
   *
   * @throws HttpIncorrectRequestException
   * @throws HttpNotFoundRequestException
