...
public abstract class HttpRequestHandler
  ...
  protected void channelRead0(ChannelHandlerContext ctx, HttpObject msg)
    ...
    try {
      if (msg instanceof HttpRequest) {
        ...
        WaarpActionLogger
            .logCreate(DbConstant.admin.getSession(), "Request received: "
        ...
        if (httpPageTemp.getPagerole() == PageRole.ERROR) {
          ...
          this.businessRequest =
              httpPage.newRequest(ctx.channel().remoteAddress());
          willClose = true;
          writeSimplePage(ctx);
          WaarpActionLogger
  ...
  protected void writeSimplePage(ChannelHandlerContext ctx)
    ...
    response.headers().set(HttpHeaderNames.CONTENT_TYPE,
                           this.businessRequest.getContentType());
    response.headers().set(HttpHeaderNames.REFERER, request.uri());
  ...
  protected void addBusinessCookie(FullHttpResponse response,
    ...
    for (AbstractHttpField field : httpPage.getFieldsForRequest(businessRequest)
                                           .values()) {
  ...
  /**
   * Method that will use the uploaded file and send back the result <br> (this
   * method must send back the answer using
   * for instance a ChunkedInput handler and should try to call clean(), but
   * taking into consideration that it will
   * erase all data, so it must be ensured that all data are sent through the
   * wire before calling it. Note however
   * that when the connection is closed or when a new request on the same
   * connection occurs, the clean method is
   * automatically called. The usage of a HttpCleanChannelFutureListener on the
   * last write might be useful.)
   *
   * @param ctx
   */
