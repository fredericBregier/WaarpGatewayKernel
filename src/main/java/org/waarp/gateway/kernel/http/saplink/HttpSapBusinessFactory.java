...
public abstract class HttpSapBusinessFactory extends HttpBusinessFactory {
   ...
   * SecurityOptional. If one SapArg is in the array, it
   * has the status associated with the rank. One SapArg can appears in multiple
   * rank.
   */
    ...
    try {
      ...
      pages.put(uri,
                new HttpPage(pagename, null, header, footer, beginform, endform,
                             nextinform,
                             uri, pageRole, errorpage, classname,
                             linkedHashMap));
      // get
      pageRole = PageRole.GETDOWNLOAD;
  ...
  private static void addDefaultFields(
    ...
    for (int j = 0; j < fields[rank].length; j++) {
      ...
      fieldrank = ++nb;
      linkedHashMap
          .put(fieldname, new DefaultHttpField(fieldname, fieldRole, fieldinfo,
                                               fieldvalue,
                                               fieldvisibility, fieldmandatory,
                                               fieldcookieset, true,
                                               FieldPosition.URL,
                                               fieldrank));
    }
   ...
   * and update exist in two modes (Post and
   * Put)
   */
