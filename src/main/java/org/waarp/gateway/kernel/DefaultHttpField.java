...
public class DefaultHttpField extends AbstractHttpField {
  ...
  public DefaultHttpField(String fieldname, FieldRole fieldtype,
                          String fieldinfo,
                          String fieldvalue, boolean fieldvisibility,
                          boolean fieldmandatory,
                          boolean fieldcookieset,
                          boolean fieldtovalidate, FieldPosition fieldposition,
                          int fieldrank) {
  ...
  public String getHtmlFormField(HttpPage page)
    ...
    case SUBMIT: {
      ...
      default:
        throw new HttpIncorrectRequestException(
            "Incorrect type: " + this.getFieldtype());
      }
    ...
    case BUSINESS_SELECT: {
      ...
      String[] values = source.fieldvalue.split(",");
      for (String string : values) {
        builder.append("<OPTION label=\"").append(string).append("\" value=\"")
               .append(string);
        if (fieldvalue != null && fieldvalue.length() > 0 &&
  ...
  public DefaultHttpField clone() {
    DefaultHttpField newField =
        new DefaultHttpField(getFieldname(), getFieldtype(), getFieldinfo(),
                             fieldvalue,
                             isFieldvisibility(), isFieldmandatory(),
                             isFieldcookieset(),
  ...
  public void setFileUpload(FileUpload fileUpload)
    ...
    if (getFieldtype() == FieldRole.BUSINESS_INPUT_FILE) {
      ...
      // should not be
      throw new HttpIncorrectRequestException(
          "Field with wrong type (should be File): "
          + getFieldname());
    }
  }
