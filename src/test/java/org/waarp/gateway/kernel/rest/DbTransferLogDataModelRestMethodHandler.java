...
public class DbTransferLogDataModelRestMethodHandler
  ...
  protected ArrayNode getDetailedAllow() {
    ...
    if (this.methods.contains(METHOD.GET)) {
      node2 = RestArgument.fillDetailedAllow(
          ...
          JsonHandler.createObjectNode()
                     .put(DbTransferLog.Columns.SPECIALID.name(),
    ...
    if (this.methods.contains(METHOD.PUT)) {
      ...
      node2 = RestArgument.fillDetailedAllow(METHOD.PUT, this.path + "/id",
                                             COMMAND_TYPE.UPDATE.name(),
                                             node3, node1);
      node.add(node2);
    }
    if (this.methods.contains(METHOD.DELETE)) {
      ...
      node3.put(DbTransferLog.Columns.SPECIALID.name(),
                "SPECIALID as Long in URI as " + this.path + "/id");
      node2 = RestArgument.fillDetailedAllow(METHOD.DELETE, this.path + "/id",
                                             COMMAND_TYPE.DELETE.name(),
                                             node3, node1);
      node.add(node2);
    }
    if (this.methods.contains(METHOD.POST)) {
      ...
      node2 = RestArgument
          .fillDetailedAllow(METHOD.POST, this.path, COMMAND_TYPE.CREATE.name(),
                             node3, node1);
      node.add(node2);
    }
    node2 = RestArgument.fillDetailedAllow(METHOD.OPTIONS, this.path,
